...
Spezifische Anforderungen die an eine Sicherheitssoftware gestellt werden finden können wie folgt zusammengefasst werden:
...
\begin{itemize}
  ...
  \item Vertraulichkeit - Der Zugriff auf Daten soll nur dann möglich sein, wenn die betroffene Person/Maschine berechtigt ist (minimale Berechtigungsvergabe)
  \item Usability - Einfacher Zugriff von Mensch und Maschine 
  \item Informationenklassifizierung - Informationen sollen dem Schutzniveau entsprechend geschützt werden 
  \item Sichere Anmeldeverfahren müssen zur Verfügung stehen 
...
\label{subsec:stand}
Um die in \autoref{subsec:einführung} beschriebenen Themenkomplexe der Authorisierung und Authentifizierung in Frimenumgebungen umzusetzten gibt es verschiedene Ansätze. Im folgenden sollen einige gängige Softwareprodukte, die die genannten Aufgaben erfüllen sollen vorgestellt und auf ihre Tauglichkeit zur vollumfänglichen Erfüllung der in \autoref{subsubsec:anforderung} beschriebenen Anforderung untersucht.
