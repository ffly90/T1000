...
\label{subsec:plan}
Die Infrastruktur für die virtualisierte Umgebung steht am Arbeitsplatz zu Beginn des Projekts zur Verfügung. Als Basisbetriebssystem für die verschiedenen Server wird das Linux-Derivat CentOS in der Version 7.4.1708 eingesetzt. Es sollen vier virtuell Maschinen installiert werden wobei Jeder Serverdienst, mit Ausnahme von Consul eine eigene virtuelle Maschine bekommt. Consul wird zusammen mit Vault auf dem selben Host installiert. Für das \ac{LDAP} soll außerdem noch ein Datenbank-Server aufgesetzt werden. Alle Server sollen, wie in Abbildung \vref{fig:test} gezeigt, über ein lokales Netzwerk miteinander verbunden werden.\newline
\begin{figure}[h]
	\centering
...
Die Grundstruktur wie in Kapitel \vref{subsec:plan} beschrieben wurde umgesetzt indem eine virtuelle Maschine mit CentOS installiert wurde. Die Daten der zugrundeliegenden virtualisierten Hardware sind folgende:
\begin{itemize}
...
\end{itemize} 
Die Betriebssysteminstallation wurde mit Standardeinstellungen für einen Server mit grafischer Oberfläche durchgeführt. Nach der Installation wurde mit dem Kommando \inline|sudo yum update| das Betriebssystem auf den neusten Stand gebracht. Anschließend wurde die virtuelle Maschine drei Mal kopiert, sodass vier identische Server zur Verfügung standen. 
\subsubsection{Installation Vault}
\label{subsubsec:instv}
Um Vault zu installieren gibt es zwei Möglichkeiten. Es kann der Quellcode heruntergeladen werden um daraus 


\begin{lstlisting}[caption={test long}, captionpos=b]
	#!/bin/bash
